...
- **蓝图存档列表（ComfyUI 式）**：左侧「蓝图」面板管理多套命名存档——另存为、点选 / 双击加载切换、重命名、删除，每行显示节点数与保存时间；启动时存在 `default` 存档自动恢复，否则加载预置示例图
- **蓝图列表**：左侧「节点」面板全量列出画布节点（标题 / 类型 / 运行状态色点），支持定位、重命名、删除管理，与画布选中双向同步
- **工作线程执行**：cv2 处理经框架后台任务线程池执行，不阻塞 UI；结果经 Qt 信号跨线程排队封送，预览图与节点状态实时刷新
- **图持久化**：画布序列化后经 DataProvider 保存/恢复，损坏存档自动回退预置示例图
- **跨插件 / MCP API**：九个 service_api 方法供其他插件或 MCP 客户端远程驱动管线与管理存档
...
4. 选中节点，在右侧属性面板编辑参数（如高斯核大小、阈值等）；左侧「节点」列表展示全部节点，可点击行选中画布节点，并执行「定位 / 重命名 / 删除」管理操作；
5. 点击工具条 **运行**，节点状态与耗时实时高亮，`preview` 节点结果显示在预览区；
6. 左侧「蓝图」列表管理多套命名存档：工具条或列表区的 **另存为** 把当前画布存为命名存档（重名提示覆盖确认），双击条目或选中后点 **加载** 切换画布内容，也可 **重命名 / 删除** 存档；插件启动时若存在 `default` 存档则自动恢复，否则加载预置示例图。
...
| `list_graphs()` | 枚举全部已保存图存档（名称/节点数/大小/修改时间） |
| `delete_graph(name)` | 删除指定图存档；不存在返回中文错误 |
| `rename_graph(old_name, new_name)` | 重命名图存档；重名冲突/不存在返回中文错误 |
...
├── ui/             # 视图层：BlueprintCanvas 宿主、工具条、蓝图存档列表面板、
│                   #   节点列表面板、属性面板、预览面板
