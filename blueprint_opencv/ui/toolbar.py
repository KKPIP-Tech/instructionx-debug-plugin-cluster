...
运行 / 停止 / 保存图 / 加载图 / 适应视图按钮与状态标签的纯视图组件：
...
class ToolBar(QWidget):
    ...
    信号:
        ...
        save_requested: 点击「保存图」。
        load_requested: 点击「加载图」。
    ...
    def __init__(self, parent: QWidget = None) -> None:
        ...
        self._save_button = Button("保存图", size=_BUTTON_SIZE)
        self._load_button = Button("加载图", size=_BUTTON_SIZE)
    ...
    def _build_layout(self) -> None:
        ...
        for button in (self._run_button, self._stop_button, self._save_button,
                       self._load_button, self._fit_button):
