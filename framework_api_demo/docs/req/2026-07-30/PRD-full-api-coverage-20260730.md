...
- 创建日期：2026-07-30
- 修改日期：2026-07-30
- 插件：framework-api-demo（官方示例插件，改造前 release.1.1.0，改造后 release.1.2.0）
- 文档类型：产品需求文档（PRD）
...
- **版本号建议**：本次为功能大幅扩展（API 覆盖面成倍增加 + 服务层/UI 结构性重构），按语义化建议由 `release.1.1.0` 升至 **`release.1.2.0`**（次版本号 +1，新增能力且向后兼容）。**已经开发者确认**，随本次收尾提交实施。
- 其他描述/配置文件：
