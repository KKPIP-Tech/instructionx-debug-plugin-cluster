...
class TestPluginInfoContract:
    ...
    def test_version_is_release_1_0_3(self) -> None:
        """版本号应为 release.1.0.3 的正式版 PluginVersion。"""
        version = self.info.version
        assert isinstance(version, PluginVersion)
        assert version.version_type is VersionType.RELEASE
        assert (version.major, version.minor, version.patch) == (1, 0, 3)
        assert str(version) == "release.1.0.3"
